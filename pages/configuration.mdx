...
A custom Canopy configuration is created at `config/canopy.json`. The file must be a valid JSON and must contain a `collection` property with a valid IIIF Collection URI.
