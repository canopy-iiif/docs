...
| :------------------ | :-------- | :-------------------------------- |
| `search.enabled`    | `boolean` | Whether or not to enable search.  |
| `search.flexSearch` | `Object`  | FlexSearch configuration options. |
| `search.index`      | `Object`  | Indexing option for Manifests.    |
...
Users may customize their [FlexSearch configuration](https://github.com/nextapps-de/flexsearch#options) using `search.flexSearch` to fit around the source Collection and its Manifest `label`, `metadata`, and `summary` properties. Customizations range from defining language-specific options such as `charset` and `stemmer`, as well as scoring options like `resolution` and `depth`.
